...
def get_trends_data(keywords, retries=3, timeframe='today 12-m', geo='US'):
...
def get_trends_in_batches(keywords, batch_size=3, retries=3, timeframe='today 12-m', geo='US'):
...
def get_year_data(year):
    ...
    # Get the package directory - this is key to finding files correctly
    package_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Use existing data files - now with correct path
    source_folder = os.path.join(package_dir, "data_from_gst_website")
    source_file = os.path.join(source_folder, f"df_{year_suffix}.csv")
    
    print(f"Looking for file: {source_file}")
    
    if os.path.exists(source_file):
        print(f"Found existing data file: {source_file}")
        
        # Copy file to data folder and rename as pytrends_data.csv
        shutil.copy2(source_file, pytrends_data_file)
        print(f"Copied {source_file} to {pytrends_data_file}")
        ...
        print(f"Error: No data file found for year {year}. Looked for: {source_file}")
        return None
...
if __name__ == "__main__":
    ...
    if args.year < 2020 or args.year > current_year:
        print(f"Error: Year must be between 2020 and {current_year}")
        exit(1)
    
    # Get and clean data for specified year
    cleaned_data = get_year_data(args.year)
