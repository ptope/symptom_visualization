...
def check_for_pca_images():
    ...
    pca_images = []
    default_pca_files = [
        'symptom_term_loadings.png',
        'symptom_term_clusters.png'
    ]
...
def find_latest_enhanced_csv():
    """
    Find the most recent enhanced CSV file in the data_enhanced directory
    
    Returns:
    tuple: (data_path, cluster_path) - paths to the original data CSV and cluster data CSV
    """
    # Check if data_enhanced directory exists
    if not os.path.exists('data_enhanced'):
        print("Error: data_enhanced directory not found")
        return None, None
    
    # Look for CSV files in data_enhanced directory
    csv_files = glob.glob('data_enhanced/*.csv')
    
    if not csv_files:
        print("Error: No CSV files found in data_enhanced directory")
        return None, None
...
if __name__ == "__main__":
    ...
    if not data_path:
        ...
        if not data_path:
            print("Could not auto-detect data file. Please specify with --data option.")
            exit(1)
